...
VVI Solver::solve_v2(Algorithm a) {
    ...
    // add connecting flights
    map<int,set<int>> connecting_flights;
    for (int i = 0; i < int(_flights.size()); ++i) {
        for (int f: city_flights[_flights[i].dest]) {
            if (_flights[i].end + 15 <= _flights[f/2].start) {
                add_edge(graph, edges, 2*i+1, f, 1);
                for (int conn_f : city_flights[_flights[f/2].dest]) {
                    if (_flights[f/2].end + 15 <= _flights[conn_f/2].start) {
                        connecting_flights[2*i+1].insert(conn_f);
                    }
                }
            }
        }
    ...
    // set connecitng flights
    for (int i = 0; i< int(_flights.size()); ++i) {
        for (int conn_f : connecting_flights[2*i+1]) {
            add_edge(graph, edges, 2*i+1, conn_f, 1); 
        }
    }
